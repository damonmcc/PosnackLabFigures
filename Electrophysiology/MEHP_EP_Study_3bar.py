...
#%% Main Figure Setup
fig = plt.figure(figsize=(7, 9))  # half an 8.5 x 11 page
...
axSNRT.bar(0.7, np.mean(mp_snrt), width, color='k', fill=False, yerr=np.std(mp_snrt) / np.sqrt(len(mp_snrt)),
          ecolor='k', error_kw=dict(lw=1, capsize=4, capthick=1.0))
axSNRT.plot(np.linspace(0.35,0.45, num=len(cp_snrt)),cp_snrt, 'o', color='k', mfc='none')
axSNRT.plot(np.linspace(0.65,0.75, num=len(mp_snrt)),mp_snrt, 'o', color='k', mfc='none')
plt.ylim([0, 600])
plt.xlim([0.2, 0.9])
...
xl = axSNRT.get_xlim()
xr = xl[1] - xl[0]
axSNRT.text(xl[0] - (xr * 0.4), yr * 0.96, 'B', ha='center', va='bottom', fontsize=10, fontweight='bold')

# WBCL
axWBCL.bar(0.4, np.mean(cp_wbcl), width, color=timeColor, fill=False, yerr=np.std(cp_wbcl) / np.sqrt(len(cp_wbcl)),
          ecolor='k', error_kw=dict(lw=1, capsize=4, capthick=1.0))
axWBCL.bar(0.7, np.mean(mp_wbcl), width, color=timeColor,fill=False, yerr=np.std(mp_wbcl) / np.sqrt(len(mp_wbcl)),
          ecolor='k', error_kw=dict(lw=1, capsize=4, capthick=1.0))
axWBCL.plot(np.linspace(0.35,0.45, num=len(cp_wbcl)),cp_wbcl, 'o', color='k', mfc='none')
axWBCL.plot(np.linspace(0.65,0.75, num=len(mp_wbcl)),mp_wbcl, 'o', color='k', mfc='none')
plt.ylim([0, 200])
plt.xlim([0.2, 0.9])
...
xl = axWBCL.get_xlim()
xr = xl[1] - xl[0]
axWBCL.text(xl[0] - (xr * 0.4), yr * 0.96, 'C', ha='center', va='bottom', fontsize=10, fontweight='bold')

# AVNERP
axAVNERP.bar(0.4, np.mean(cp_avnerp), width, color='k', fill=False, yerr=np.std(cp_avnerp) / np.sqrt(len(cp_avnerp)),
          ecolor='k', error_kw=dict(lw=1, capsize=4, capthick=1.0))
axAVNERP.bar(0.7, np.mean(mp_avnerp), width, color='k', fill=False, yerr=np.std(mp_avnerp) / np.sqrt(len(mp_avnerp)),
          ecolor='k', error_kw=dict(lw=1, capsize=4, capthick=1.0))
axAVNERP.plot(np.linspace(0.35,0.45, num=len(cp_avnerp)),cp_avnerp, 'o', color='k', mfc='none')
axAVNERP.plot(np.linspace(0.65,0.75, num=len(mp_avnerp)),mp_avnerp, 'o', color='k', mfc='none')
plt.ylim([0, 200])
plt.xlim([0.2, 0.9])
...
xr = xl[1] - xl[0]
axAVNERP.text(xl[0] - (xr * 0.4), yr * 0.96, 'D', ha='center', va='bottom', fontsize=10, fontweight='bold')

#%% Saving
plt.subplots_adjust(left=0.2, right=0.9, bottom=0.1, top=0.9, wspace=0.8, hspace=0.9)

#fig.savefig('MEHP_AV_Properties.svg') #Vector, for loading into InkScape for touchup and further modification
