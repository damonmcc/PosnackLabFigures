...
def example_ConductionDelay(axis, dataBase, dataPost, times):
    ...
    axis.set_xlim([min(times), min(times) + 60])
...
def example_ConductionDelay2(axis, dataBase, dataPost):
    timesBase = dataBase[:, 1] * 1000  # seconds to ms
    timesPost = dataPost[:, 1] * 1000  # seconds to ms
    timeWindow = 50
    axis.set_xlabel('Time (ms)', fontsize=12)
    axis.tick_params(axis='x', which='both', direction='in', bottom=True, top=False)
    axis.set_xlim([min(dataBase[:, 1]), min(dataBase[:, 1]) + timeWindow])
    axis.xaxis.set_major_locator(ticker.MultipleLocator(20))
    axis.xaxis.set_minor_locator(ticker.MultipleLocator(10))
    axis.set_ylabel('Vm (Normalized)', fontsize=12)
    axis.tick_params(axis='y', which='both', direction='in', right=False, left=True)
    ...
    shiftX = -715
    shiftPost = -23
    axis.plot(timesBase, np.roll(dataBase[:, 0], shiftX),
              color=colorBase, linewidth=2, label='Base')
...
axActMap140CTRL.set_title('Base', weight='semibold')
axActMap140MEHP.set_title('Post', weight='semibold')
...
example_ActMapMEHP(axActMap140MEHP, actMapsMEHPpost[140])
example_ActMapCTRL(axActMap190CTRL, actMapsMEHPbase[190])
example_ActMapMEHP(axActMap190MEHP, actMapsMEHPpost[190])
example_ActMapCTRL(axActMap240CTRL, actMapsMEHPbase[240])
example_ActMapMEHP(axActMap240MEHP, actMapsMEHPpost[240])
...
example_ConductionDelay2(axConDelaysMEHP, ConDelayMEHP_base140, ConDelayMEHP_post140)
